...
within the $\pm$1.5/$\pm$1.0~wt\% bands (spike 16). Large per-phase
...
renormalised at its normalized published share (spike 16). No phase is
...
the target of a planned Brindley-correction extension (spike 17 of the
repository roadmap). Spike 16 validates the suite independently
(\texttt{benchmarks/spikes/spike\_16\_validate.py}): two full re-runs
...
\texttt{make report} (result-JSON md5 recorded). The spike-16 harness
...
the acceptance gate of the repository roadmap (spike 17 Brindley
