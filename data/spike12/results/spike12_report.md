...
- Two-stage verification: (1) fast kinematic intensity calc from the COD CIF (gaussian-atom form factors, exact orbit/absorption via the site sum) ranks all screen survivors by coverage + intensity correlation; (2) the top 8 per sample plus the top 5 by screen significance are confirmed with full GSAS-II CIF simulation (spike06 protocol, Cu Kα1/α2) on the shared d-space grid.
...
- Diagnosis (`benchmarks/spikes/periclase_diag.py` → `data/spike12/work/periclase_diag.json`): the strong MgO lines 200 (d=a/2) and 220 (d=a/√8) land within 0.003 Å of a picked measured peak in **all four** samples (exact offsets +0.0004..−0.0013 Å). No systematic shift, no zero-error, no wavelength mis-set.
