"""Unit 09: multi-phase (2-phase) measured pattern through the M1 pipeline.
...
pipeline -- this unit documents what the single-phase engine actually does
...
UNIT = ROOT / "data" / "unit09"
IN_DIR = UNIT / "input"
RES_DIR = UNIT / "results"
WORK_DIR = ROOT / "data" / "unit08" / "work"
POLICY_PATH = ROOT / "governance" / "policies" / "refinement-budget.v1.json"
REGISTRY_PATH = ROOT / "data" / "unit03" / "results" / "registry.json"
...
def main() -> int:
    ...
    for mix in MIXTURES:
        ...
        for comp in mix["components"]:
            cif = ROOT / "data" / "unit06" / "input" / "cod" / comp["cif"]
        ...
        # --- statistics gate: the measurement must support the claim (unit 05)
    ...
    report = {
        ...
        "observation": (
            ...
            "the code -- this unit documents the empirical gap."),
    }
    (RES_DIR / "unit09_report.json").write_text(json.dumps(report, indent=2))

    print(f"\nreport      : {RES_DIR / 'unit09_report.json'}")
    return 0
