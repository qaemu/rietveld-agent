"""Unit 02: generate golden XRDML fixtures using the validated GSAS-II
simulator (see unit 01), and emit the unit report + reproducibility check.
...
Run modes:
  python 02_generate_fixtures.py [--simulate]   # (re)generate + report
  python 02_generate_fixtures.py --verify       # hash-check goldens vs report
"""
...
UNIT_DIR = os.path.join(ROOT, "data", "unit02")
IN_DIR = os.path.join(UNIT_DIR, "input")
WORK_DIR = os.path.join(UNIT_DIR, "work")
RES_DIR = os.path.join(UNIT_DIR, "results")
FIX_DIR = os.path.join(ROOT, "tests", "fixtures", "xrdml")
VENDOR = os.path.join(ROOT, ".vendor", "GSAS-II")

CU_PRM_SRC = os.path.join(ROOT, "data", "unit00", "input", "INST_XRY.PRM")
PB_CIF_SRC = os.path.join(ROOT, "data", "unit00", "input", "PbSO4-Wyckoff.cif")
...
def main() -> None:
    ...
    report_json = os.path.join(RES_DIR, "unit02_report.json")
    if args.verify:
    ...
    # markdown summary
    md = ["# Unit 02: XRDML ingest + fingerprinting",
          "",
    ...
    with open(os.path.join(RES_DIR, "unit02_report.md"), "w") as fh:
        fh.write("\n".join(md) + "\n")
