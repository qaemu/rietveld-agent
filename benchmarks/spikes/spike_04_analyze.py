"""Spike 04 (b): M1 vertical slice -- analyze everything, record bundles.
...
Each case produces a schema-validated RunBundle under data/spike4/results/
"""
...
RES_DIR = os.path.join(ROOT, "data", "spike4", "results")
...
def main() -> None:
    ...
    with open(os.path.join(RES_DIR, "spike04_report.json"), "w") as fh:
    ...
    md += [
        ...
        "- SUPERSEDED by spike 05: the 0.108 cross-anode margin shown in M1 was "
        ...
        "see data/spike5/results/spike05_report.md. M1 verdicts unchanged, "
        "same-anode and cross-anode both hold.",
        "- Catalogue-backing (spike 06): the M1 sampling of quartz-family "
    ...
    with open(os.path.join(RES_DIR, "spike04_report.md"), "w") as fh:
        fh.write("\n".join(md) + "\n")
    print("wrote", os.path.join(RES_DIR, "spike04_report.json"))
    if not report["n_all_passed"]:
        raise SystemExit("spike04: at least one case failed")
