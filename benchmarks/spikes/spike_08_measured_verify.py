"""Spike 08: bounded verification on MEASURED patterns + noise envelope.

Wires the spike-07 verification engine into the real pipeline (cli/analyze)
...
(spike 05's deterministic protocol). For every replicate the true phase and
...
Run: python benchmarks/spikes/spike_08_measured_verify.py
"""
...
WORK_DIR = os.path.join(ROOT, "data", "spike8", "work")
RES_DIR = os.path.join(ROOT, "data", "spike8", "results")
...
#: deterministic envelope levels (spike 05 protocol ids / counts / s_mm / bg)
ENVELOPE_LEVELS = [
...
def main() -> None:
    ...
    with open(os.path.join(RES_DIR, "spike08_report.json"), "w") as fh:
        json.dump(report, fh, indent=2)

    lines = [
        "# Spike 08: bounded verification on measured patterns",
        "",
    ...
    with open(os.path.join(RES_DIR, "spike08_report.md"), "w") as fh:
        fh.write("\n".join(lines) + "\n")
    print(f"[done] spike08 report -> {RES_DIR} "
