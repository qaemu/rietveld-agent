"""Spike 12: match any powder file against the ENTIRE COD database.
...
Pipeline per input file:
  ...
  3. take the top screen hits, download their CIFs from crystallography.net,
     and re-rank them by full GSAS-II CIF simulation (deterministic spike06
...
COD_CSV_SRC = Path("/var/folders/fv/gdlbbrkd1vz55yw6slf795dc0000gn/T/opencode/mp/cod_metadata.csv")
...
CIF_DIR = ROOT / "data" / "spike12" / "work" / "cifs"
RES_DIR = ROOT / "data" / "spike12" / "results"
...
def ensure_cod_csv() -> Path:
    ...
    if COD_CSV_SRC.exists():
        import shutil
        shutil.copy2(COD_CSV_SRC, dst)
        return dst
    print(f"[cod] downloading full COD metadata export ({COD_CSV_URL}) ...")
...
def main() -> int:
    ...
    ap.add_argument("--report-only", action="store_true",
                    help="regenerate spike12_report.md from the saved JSON "
                         "(no screening/verification)")
    args = ap.parse_args()

    if args.report_only:
        rep = json.loads((RES_DIR / "spike12_report.json").read_text())
        results = rep["verification"]
        write_report_md(rep, results)
        print(f"[report-only] {RES_DIR / 'spike12_report.md'} regenerated "
              f"from spike12_report.json ({len(results)} entries)")
        return 0
    ...
    hkl_cache_dir = ROOT / "data" / "spike12" / "work" / "hkl_cache"
    ...
    work_dir = ROOT / "data" / "spike12" / "work" / "sim"
    ...
    (RES_DIR / "spike12_report.json").write_text(
...
def write_report_md(report: dict, results: dict) -> Path:
    """Regenerate spike12_report.md from a saved report (re-runnable)."""
    files = [Path(n) for n in report["samples"]]
    md = [
        "# Spike 12: whole-COD matching (with any file given)",
        "",
        ...
        "(spike06 protocol, Cu Kα1/α2) on the shared d-space grid.",
    ...
    md.append("- Diagnosis (`benchmarks/spikes/periclase_diag.py` → "
              "`data/spike12/work/periclase_diag.json`): the strong MgO "
    ...
    (RES_DIR / "spike12_report.md").write_text("\n".join(md))
    print(f"\n[report] {RES_DIR / 'spike12_report.md'}")
    ...
    return RES_DIR / "spike12_report.md"
