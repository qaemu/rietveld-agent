"""Spike 06: COD catalog release v0.1.1 + catalog-backed candidate library.
...
Audit (chemistry validation caught during spike 05/06 review)
-------------------------------------------------------------
...
* ``9012601`` in the "SiO2 (quartz)" family was a **compressed quartz
  ...
  in ``data/spike6/input/cod/`` and this audit documents why).

Run:  python benchmarks/spikes/spike_06_catalog_build.py
"""
...
WORK_DIR = os.path.join(ROOT, "data", "spike6", "work")
RES_DIR = os.path.join(ROOT, "data", "spike6", "results")
...
UA = {"User-Agent": "rietveld-agent-spike06/0.1 (research; local eval)"}
...
TARGETS = [
    ...
    {"cod_id": 1000041, "mineral": "halite", "family": "NaCl (halite)",
     ...
     "local": os.path.join(ROOT, "data", "spike4", "input", "halite_1000041.cif")},
    {"cod_id": 9011025, "mineral": None, "family": "AgBiS2 (schapbachite)",
     "formula": {"Ag": 0.5, "Bi": 0.5, "S": 1.0}, "sg_hint": "F m -3 m",
     "local": os.path.join(ROOT, "data", "spike4", "input", "halite_9011025.cif"),
    ...
    {"cod_id": 1009000, "mineral": None, "family": "GaAsO4 (quartz homeotype)",
     ...
     "local": os.path.join(ROOT, "data", "spike2", "input", "quartz_1009000.cif"),
...
def main() -> None:
    ...
    # structures new to the project (not among pre-spike-06 fixture CODs)
    ...
    payload = {
        ...
        "version": "0.1.1",
        "state": "draft",
        "built_by": "spike-06 (rietveld-agent)",
    ...
    legacy = [
        {"id": "mat-pbso4", "name": "PbSO4", "phase_family": "PbSO4 (anglesite)",
         ...
         "provenance": "APS GSAS-II tutorial CIF (spike 02 fixture); "
    ...
    report = {
        "spike": "spike_06", "release": "catalog_0.1.1",
    ...
    with open(os.path.join(RES_DIR, "spike06_report.json"), "w") as fh:
    ...
    lines = [
        ...
        f"(first-introduced by spike 06: all but the three pre-existing "
    ...
    lines += ["",
              ...
              "- Spike 04/05 re-run on the catalog-backed library: e2e green "
    ...
    with open(os.path.join(RES_DIR, "spike06_report.md"), "w") as fh:
        fh.write("\n".join(lines) + "\n")
    print(f"[done] spike06 report -> {RES_DIR}")
