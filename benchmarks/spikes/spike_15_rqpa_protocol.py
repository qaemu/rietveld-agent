"""Spike 15: publication-grade RQPA protocol runner for SRM 2686a.

Implements docs/rqpa_protocol.md on the four published NIST SRM 2686a
patterns (same .xye re-emission as spikes 11-13, intensities untouched):

  * phase models: the spike-14 published structure set
...
Report: data/spike15/results/spike15_report.{json,md} with per-phase wt%
against published, Rwp, convergence, tier, and the md5 of the result JSON
for the spike-16 reproducibility check.
"""
...
WORK = ROOT / "data" / "spike15" / "work"
RES = ROOT / "data" / "spike15" / "results"
...
#: and the reduced T1 subcell (1538413, spikes 12-13). Reported side by side
    ...
    for fname in SAMPLES:
        for model, (aname, acod) in ALITE_MODELS.items():
            ...
            if fname == "Clinker_Synchrotron.dat" and model == "T1":
                ...
                # report sync on the M3 model.  See notes/spike15.md.
                continue
...
def write_md(results: list, md5: str) -> None:
    ...
    for res in results:
        ...
        lines.append("")
    (RES / "spike15_report.md").write_text("\n".join(lines))
    print(f"\nwrote {RES / 'spike15_report.json'} and .md")
    return 0
