"""Spike 16: validation harness for the spike-15 RQPA protocol.

Three independent checks against the committed protocol
(benchmarks/spikes/spike_15_rqpa_protocol.py, docs/rqpa_protocol.md):

  A. Reproducibility -- rerun the full spike-15 suite from scratch and
  ...
  B. Synthetic ground-truth recovery (known-answer test).  For each sample
     ...
     whose phase fractions are, by construction, the published values:
       ...
       4. run the *unmodified* spike-15 ladder on y_synth from the
  ...
  C. Gate scoring on the real results (fresh report from A):
       ...
       * operational: converged, no marker errors, wR < 25 (spike-15 gate)

Report: data/spike16/results/spike16_report.{json,md}.  Exit 0 whenever
...
CLI: --skip-rerun (validate against the existing spike-15 report, no
...
import spike_15_rqpa_protocol as spike15                        # noqa: E402

SPIKE15 = ROOT / "benchmarks" / "spikes" / "spike_15_rqpa_protocol.py"
RES15 = ROOT / "data" / "spike15" / "results" / "spike15_report.json"
WORK15 = ROOT / "data" / "spike15" / "work"
RES16 = ROOT / "data" / "spike16" / "results"
WORK16 = ROOT / "data" / "spike16" / "work"
STRU = ROOT / "data" / "structures"
PRM_CU_SRC = ROOT / "data" / "spike11" / "work" / "INST_XRY_CU_CLINKER.PRM"
CU_WL, SYNC_WL = spike15.CU_WL, spike15.SYNC_WL
...
def content_hash(samples: list) -> str:
    ...
    match spike-15's _content_hash()."""
...
def legacy_content_hash(samples: list) -> str:
    """Hash as spike-15 locked it before spike 16 (timing INCLUDED)."""
...
def check_reproducibility(skip_rerun: bool) -> dict:
    ...
    if not skip_rerun:
        t0 = time.time()
        log = Path("/tmp") / f"spike16_suite_rerun_{int(t0)}.log"
        with open(log, "w") as fh:
            rc = subprocess.call([sys.executable, str(SPIKE15)],
...
def ground_truth(fname: str) -> dict:
    ...
    wt = spike15.published_norm(fname)
    if fname in spike15.MERGE_ALUMINATE:
...
def materialize_ycalc(fname: str, prm: Path, xye_real: Path, wt: dict,
    ...
    phases = spike15.PHASESETS[fname]
    tag = fname.split(".")[0]
...
def run_kat(fname: str, prm: Path, *, counts: int, seed: int) -> dict:
    ...
    base = spike15.PHASESETS[fname]
    attempts = [("", None)]
    if fname in spike15.CONSTRAINED_TRACE:
        attempts.append(("constrained", spike15.CONSTRAINED_TRACE[fname]))
    if fname in spike15.DROP_FALLBACK:
        attempts.append(("drop", list(spike15.DROP_FALLBACK[fname])))
    scale_init = fname in spike15.INIT_SCALES
    res, fallback_used = None, False
    constraints = []
    old_work = spike15.WORK
    spike15.WORK = WORK16
    try:
        for atag, dropped in attempts:
            exclude = (spike15.CONSTRAINED_TRACE[fname]
                       if atag == "constrained" else
                       (spike15.DROP_FALLBACK[fname] if atag == "drop"
                        else []))
            constrained = (list(exclude)
                           if atag == "constrained" else None)
            spike15.PHASESETS[fname] = (
                [(n, c) for (n, c) in base if n not in exclude]
                if exclude else base)
            try:
                proj, h, obs, rwp_norm, wr, converged, bad, lst_txt, lo2, \
                    hi2, stage_log = spike15._build_refine(
                        fname, prm, xye, stages=spike15.STAGES[fname],
                        cycles=spike15.MAX_CYCLES, scale_init=scale_init)
            except Exception as e:                      # noqa: BLE001
                spike15.PHASESETS[fname] = base
            ...
            res = spike15._extract(fname, "synthetic", proj, h, rwp_norm, wR,
            ...
            if converged and not bad and wR < spike15.WR_GOOD:
                break
    finally:
        spike15.WORK = old_work
        spike15.PHASESETS[fname] = base
    if res is None:
    ...
    if constraints:
        ...
        res["phases_dropped"] = list(spike15.DROP_FALLBACK[fname])
    res["compared"] = spike15.compare(res)
...
def gate_row(res: dict) -> dict:
    ...
    g = {
        ...
        and (res.get("wR") or 100.0) < spike15.WR_GOOD,
    }
...
def rank_report(res: dict, rows: list) -> dict:
    ...
    if res["sample"] in spike15.MERGE_ALUMINATE:
        ours = {p["phase"]: p["wt_frac"]
                for p in spike15.merge_alu(res["phases"])}
...
def write_report(repro, kats, gates, *, skip_synth: bool) -> dict:
    ...
    js = {
        "harness": "benchmarks/spikes/spike_16_validate.py",
    ...
    (RES16 / "spike16_report.json").write_text(json.dumps(js, indent=2))
    lines = ["# Spike 16: validation of the spike-15 RQPA protocol", "",
             f"harness: benchmarks/spikes/spike_16_validate.py | "
    ...
    (RES16 / "spike16_report.md").write_text("\n".join(lines))
    print(f"wrote {RES16 / 'spike16_report.json'} and .md")
    return js
...
def main() -> int:
    ...
    ap.add_argument("--skip-rerun", action="store_true",
                    help="do not rerun the spike-15 suite")
    ...
    if not args.skip_synth:
        prm_cu = spike15.write_prm(*CU_WL, 0.0,
                                   WORK16 / "INST_CU_PROTOCOL.PRM")
        prm_sync = spike15.write_prm(SYNC_WL, SYNC_WL, 0.0,
