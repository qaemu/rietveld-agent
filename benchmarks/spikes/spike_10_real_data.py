"""Spike 10: OUT-OF-SAMPLE validation on real, published RRUFF powder XRD.
...
Honest framing (no data fabrication):
  ...
  * the RRUFF instrument (Cu Ka1 1.540598 / Ka2 1.544426 per tube
    ...
    (data/spike10/work/registry_rruff.json); the shipped registry under
    data/spike3/ is NOT modified;
  ...
  * verification runs against the RRUFF grid via resampling onto the
    ...
    statistics gate (spike-05 envelope L3: 1e5 peak counts) is ASSESSED
...
IN_DIR = ROOT / "data" / "spike10" / "input"
WORK_DIR = ROOT / "data" / "spike10" / "work"
RES_DIR = ROOT / "data" / "spike10" / "results"
...
def _registry_with_rruff() -> tuple:
    ...
    rec = {
        ...
        "wavelength": wl,
        "state": "released",
        "approval": {"reviewed_by": "spike-10-validation",
        ...
        "content": {"gsasii_instprm": {"path": str(prm_path.relative_to(ROOT)),
                                       ...
                                       "format": "GSAS-II instrument "
                                                 "parameter file"},
                    "notes": "Validation-only registration (spike 10): RRUFF "
...
def main() -> int:
    ...
    report = {
        ...
        "honest_limitations": [
            ...
            "counting-statistics floor for a refinement claim (spike-05 "
            ...
            "registry (data/spike10/work/registry_rruff.json); the shipped "
            "registry under data/spike3/ is untouched.",
    ...
    (RES_DIR / "spike10_report.json").write_text(json.dumps(report, indent=2))

    md = ["# Spike 10 - out-of-sample validation: RRUFF real powder XRD",
          "",
    ...
    md += ["",
           ...
           "1e5 (the spike-05 calibrated floor for a refinement claim), so "
           ...
           "identification."]
    (RES_DIR / "spike10_report.md").write_text("\n".join(md))
    print(f"report: {RES_DIR / 'spike10_report.md'}")
    return 0
