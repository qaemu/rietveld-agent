"""Spike 05: threshold-robustness evaluation for the M1 fingerprint thresholds.
...
Output
------
data/spike5/results/spike05_report.{json,md} with per-material stats
...
Run:  python benchmarks/spikes/spike_05_eval_thresholds.py
"""
...
RES_DIR = os.path.join(ROOT, "data", "spike5", "results")
FIX_DIR = os.path.join(ROOT, "tests", "fixtures", "xrdml")
REGISTRY = os.path.join(ROOT, "data", "spike3", "results", "registry.json")
...
def _halite_pattern(work_dir: str) -> PowderPattern:
    ...
    Spike 06 chemistry validation showed the old 'halite_9011025.cif'
    (used by spike 04/05) is Ag0.5Bi0.5S, so the eval material is the
    ...
    cif = os.path.join(ROOT, "data", "spike4", "input", "halite_1000041.cif")
...
def main() -> None:
    ...
    work = os.path.join(ROOT, "data", "spike5", "work")
    ...
    with open(os.path.join(RES_DIR, "spike05_report.json"), "w") as fh:
    ...
    lines = [
        ...
        f"introduced by spike 05: statuses are per phase family, so "
    ...
    with open(os.path.join(RES_DIR, "spike05_report.md"), "w") as fh:
        fh.write("\n".join(lines) + "\n")

    print()
    print(verdict_text)
    print("wrote", os.path.join(RES_DIR, "spike05_report.json"))
