# Spike 15: publication-grade RQPA runner -- run log and locked decisions

Runner: `benchmarks/spikes/spike_15_rqpa_protocol.py`
Result artifacts: `data/spike15/results/spike15_report.{json,md}` (md5
...
spike-14 published polymorph set, dual alite model (M3 / T1), Hill-Howard
...
scriptable), work files in `data/spike15/work/` (`.gpx` ignored by git,
...
scales x1e12). Cells stay at the spike-14 published values.
...
corrections in spike 17; spike 16 validates reproducibility/rank
structure with a synthetic-pattern harness.

Open items for spikes 16-17: gate wR <= 6.5 (Cu) / <= 5 (sync) vs
