...
Status: **complete** (results: `data/spike16/results/spike16_report.{json,md}`).
...
Harness: `benchmarks/spikes/spike_16_validate.py` — three independent
checks; exit 0 whenever the harness completes (verdicts are data).

## A. Reproducibility

Procedure: rerun the full spike-15 suite (6 runs, ~15 min on this Mac)
...
spike-15 locks that canonical hash (`spike_15._content_hash`).
...
- the original spike-15 hash included `elapsed_s`, so every rerun locked
  a new hash even though only the timing differed; canonicalized in
  spike-15 (`_content_hash`, timing stripped) and mirrored in spike-16
...
3. the UNMODIFIED spike-15 ladder runs on `y_synth` from the protocol's
   own starts (incl. `INIT_SCALES`), with the same attempt/constraint/
   fallback logic as `spike_15.main()`;
...
- **ferrite / alu**: the previous 4/5 result (ferrite recovered 67.82 vs
  ...
  `published_norm()` is now the shared comparison reference (spike-15
...
- The real-data alu fit also gained a cell + breadth ladder (stable
  ...
  8.4 -> 17.4 wt% (remainder of the gap is microabsorption — spike 17).
...
dominant residual is microabsorption, see spike 17).

## Next steps (roadmap)

- Spike 17: Brindley microabsorption correction (the dominant
...
- Re-check gates after spike 17; re-run B (KAT) to confirm the
...
python3 benchmarks/spikes/spike_16_validate.py --skip-rerun --sample aluminate_enriched_residue_clinkerNIST_180718_R1.xrdml
```
