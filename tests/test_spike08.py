"""Spike 08 tests: bounded verification on MEASURED patterns (numpy engine).
...
Exercises the measured-path verification wired into cli/analyze:
  ...
  * seeded noise envelope: the true phase stays lowest-Rwp at every level,
    and the calibrated bound is not stricter than the spike-07 policy bound
...
data/spike8/work (fixed CIF + protocol -> fixed pattern).
"""
import json
import os
import sys
...
from benchmarks.eval.sim import (ensure_gsasii,                        # noqa: E402
                                 sim_cif_to_pattern)
from benchmarks.spikes.spike_02_generate_fixtures import build_xrdml   # noqa: E402
...
CID_CIF = ROOT / "data" / "spike6" / "input" / "cod"
RELEASE = ROOT / "data" / "catalog" / "releases" / "catalog_0.1.1.json"
WORK = ROOT / "data" / "spike8" / "work"
...
class TestEnvelopePolicyConsistency(unittest.TestCase):
    def test_policy_bound_was_not_relaxed_beyond_envelope(self):
        # the spike-08 envelope (L1..L5 x3 seeds) calibrates the bound from
        # the true-phase Rwp max + margin: it must not be stricter than the
        # spike-07 bound (0.35) - the report records the calibrated value
        report = json.loads(
            (ROOT / "data" / "spike8" / "results" /
             "spike08_report.json").read_text())
