"""Unit 05 tests: noise-model determinism, family-aware margin semantics,
...
REPORT = ROOT / "data" / "unit05" / "results" / "unit05_report.json"
...
class TestFamilyMargin(unittest.TestCase):
    """Unit 05 core finding: differently-realized entries of the SAME phase
...
class TestVerdictCountingIndependence(unittest.TestCase):
    ...
    def test_gate_floor_lives_in_verification(self):
        # the calibrated floor (unit-05 envelope L3) guards the refinement
...
class TestPolicyRecord(unittest.TestCase):
    ...
    def test_record_consistent_with_verdict_defaults(self):
        ...
        self.assertIn("unit05_report.json", rec["evidence"])
...
class TestEvalReport(unittest.TestCase):
    ...
    def test_report_markdown_exists(self):
        self.assertTrue((REPORT.parent / "unit05_report.md").exists())
