"""Spike 07 tests: bounded Rietveld verification.
...
WORK = ROOT / "data" / "spike7" / "work"
RES = ROOT / "data" / "spike7" / "results"
FIX = ROOT / "tests" / "fixtures" / "xrdml"
RELEASE_STATE = ROOT / "data" / "spike4" / "results" / "bundles"
...
class TestBundleSchemaAdditive(unittest.TestCase):
    def test_existing_bundle_still_valid(self):
        ...
        bundles_dir = ROOT / "data" / "spike4" / "results" / "bundles"
    ...
    def test_verification_evidence_embedded_in_bundle(self):
        # verification outcome from spike 07 embeds into a real run bundle
        report = json.loads((RES / "spike07_report.json").read_text())
        case = report["cases"][0]
        ...
        reg = CalibrationRegistry.load(str(ROOT / "data" / "spike3" / "results"
                                           / "registry.json"))
